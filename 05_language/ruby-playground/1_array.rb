...
# %w는 문자열 배열을 쉽게 다루는 문법이다.
# %w 외에도 %w, %W, %i, %I 등이 존재한다.
%w[a b a c b] # ["a", "b", "a", "c", "b"]
%i[a b a c b] # [:a, :b, :a, :c, :b]

# 대문자 리터럴은 인터폴레이션이 가능한지다.
x = 10
%w[x #{x}] # ["x", "\#{x}"]
%W[x #{x}] # ["x", "10"]
%i[x #{x}] # [:x, :"\#{x}"]
...
# 구분자는 [] 뿐만 아니라 여닫는 문자만 맞으면 무엇이든 사용할 수 있다.
%w{1 2} # ["1", "2"]
%w(1 2) # ["1", "2"]
%w|1 2| # ["1", "2"]

# 공백이 기준이기 때문에, 연속 공백은 의미 없다.
%w[1              2] # # ["1", "2"]

# 공백을 포함하고 싶다면, escape 를 사용하면 된다.
%w[1\ \ \ \ \  2] #["1     ", "2"]
